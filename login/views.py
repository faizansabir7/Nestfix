...
def spregister(request):
    if request.method == "POST":
        ...
        user = User.objects.create_user(username, email, password)
        user.phno=tel
        sp=ServiceProvider(user=user,service=service,district=district,city=city)
        sp.save()
...
def getsp(request,username):
    ...
    if request.method=='POST':
        sp=ServiceProvider.objects.get(user=User.objects.get(username=username))
        if Booking.objects.filter(user=request.user, sp=sp, complete=False).exists():
            ...
            a.save()
            form_submitted = True
            messages.success(request, 'Congratulations! Your Booking is successful. Our Service Provider will contact you soon.')
    ...
    return render(request,'login/getsp.html',{
        ...
        "comments":Comment.objects.filter(sp=ServiceProvider.objects.get(user=User.objects.get(username=username))),
...
def orders(request):
    ...
    if request.GET.get('link'):
        complete_orders = Booking.objects.filter(complete=True,user=request.user)
        pending_orders = Booking.objects.filter(complete=False,user=request.user)
...
def comment(request,bid):
    ...
    if request.method=="POST":
        ...
        a=Comment(sp=booking.sp,comment=comment,user=request.user,rate=rating)
        a.save()
